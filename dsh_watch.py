...
Data sources:
  1. ~/.dsh/sessions/<workspace>/<session-id>/session.jsonl.zstd
     ...
     used to determine precisely:
       - waiting_user: the model called the ask_user_question tool and is
         suspended (user question), or an approval/asked event is pending,
         -> only then do we report "needs user action"
       - turn_done: a turn/end event -> report "done" only when a whole work
         turn has actually finished
     (an older version mistakenly treated pendingCalls -- ordinary tools being
     executed -- as waiting for the user; that has been fixed)
  2. ~/.dsh/storages/session_projcache.json title projection (session titles)

Event semantics based on @deepseek-ai/dsh-session-stats and dsh-user-approval sources:
  tool/call  -> data.callId enters pendingCalls, data.name is the tool name
  tool/result-> the matching callId leaves
...
# Only track session files written within this many hours (archived sessions
...
def _find_call_id(ev):
    """Extract callId from an event (compatible with different event shapes)."""
    d = ev.get("data")
...
class SessionTrace:
    ...
    def __init__(self):
        ...
        self.pending = {}         # callId -> tool name (ordinary tool running)
        self.waiting = False      # waiting for the user (question / approval pending)
        self.waiting_kind = ""    # question | approval
        self.processed_lines = 0  # number of folded event lines
        self.title = ""
        self.mtime = 0.0
        self.size = -1
        self.path = ""
...
class DshWatch:
    ...
    def _list_session_files(self):
        """Enumerate recently active session log files; returns {session_id: SessionTrace}."""
        out = {}
        ...
        for ws_name in os.listdir(self.sessions_dir):
            ...
            for sid in os.listdir(ws_dir):
                ...
                try:
                    ...
                    continue
                # Skip archived / long-idle sessions (only track those written in
                # the last 24 hours)
    ...
    def _read_events(self, path):
        """Decompress the whole log and parse it into an event list; None if corrupt."""
        try:
    ...
    def _fold(self, trace, events, emit):
        """Fold new events starting from trace.processed_lines; the first full fold
        only builds a baseline and does not emit."""
    ...
    # ---------- polling ----------
    def poll(self):
        """Poll once; returns the list of new events this round (also dispatched
        ...
        for sid, trace in sessions.items():
            ...
            if evs is None:
                continue
            # File was rewritten (line count shrank) -> reset the baseline
    ...
    def summary(self):
        """Per-session state summary (event-stream state + projected title).

        Anything pending for more than 1 hour counts as idle: avoid forever
        showing a long-forgotten unanswered question as "waiting for you".
        """
